...
trab <- pca(r=ko[,c("TbSp","TbN")], nfactors = 1, scores = T)
stren <- pca(r=ko[,c("MaxLoad","FailLoad")], nfactors = 1, scores = T)
...
ColoredNodes <- buildColorDict(massC,trabC,strenC)
dsub_set <- ko
title <- "Ko - PCA synthetic"

# Clean up unused variables
rm(list=c("ko1","ko13","ko134","ko3","ko4","pko1","pko3","pko4","nastring","massC","trabC","strenC"))
